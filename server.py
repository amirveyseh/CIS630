...
parser.add_argument('--data_dir', type=str, default='../dataset/doc')
parser.add_argument('--vocab_dir', type=str, default='../dataset/event')
...
def prepare():
    ...
    return [opt, vocab, emb_matrix]


# @app.route('/load_data', methods=['POST'])
def load_data(low, high, opt, vocab):
    # global opt
    # global vocab
    # low = int(request.form['low'])
    # high = int(request.form['high'])
    print("Loading data from {} with batch size {}...".format(opt['data_dir'], opt['batch_size']))
    train_batch = DataLoader(opt['data_dir'] + '/train.json', opt['batch_size'], opt, vocab, evaluation=False, low=low,
                             high=high)
    dev_batch = DataLoader(opt['data_dir'] + '/dev.json', opt['batch_size'], opt, vocab, evaluation=True, low=0,
                           high=-1)
...
def train(train_batch, dev_batch, opt, vocab,emb_matrix, round):
